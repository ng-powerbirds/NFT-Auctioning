...
const defaults = {defaultFundAmt: '20', defaultReserveAmt: '2', defaultTokenID: '1', defaulttime: '20',standardUnit};
...
class App extends React.Component {
  ...
  async componentDidMount() {// Hook into rects componentDidMount lifecycle event, which is called when the compnent starts 
    ...
    if (await reach.canFundFromFaucet()) {// see if we can access the Reach developer testing network 
      ...
      this.setState({view: 'CreatorOrBidder'});// If canFundFaucet was false  we set the component to skip 
    }
  }
  async fundAccount(fundAmount) {// we define what happens when the user clicks the Fund Account button 
    ...
    this.setState({view: 'CreatorOrBidder'});// we set the component state to display 
  }
  async skipFundAccount() { this.setState({view: 'CreatorOrBidder'}); }// we define what to do when the user clicks the skip button, which is to set the component state to display 
  selectBidder() { this.setState({view: 'Wrapper', ContentView: Bidder}); }// we set the sub component based on whether the user clicks Seller or Bidder
...
class Creator extends Common {
  constructor(props) {
    super(props);
    this.state = {view: 'SetParameters'};
  }
  setParameters(getSale) { this.setState({view: 'Deploy', getSale}); }
  // async deploy() {
  //   const ctc = this.props.acc.contract(backend);
  //   this.setState({view: 'Deploying', ctc});
  //   this.getSale = reach.parseCurrency(this.state.getSale); // UInt
